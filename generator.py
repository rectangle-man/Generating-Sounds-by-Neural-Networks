...
MIN_MAX_VALUES_PATH = "D:/Projects/Generating Sounds by Neural Networks/fsdd/min_max_values.pkl"
...
def load_fsdd(spectrograms_path):
    ...
    for root, _, file_names in os.walk(spectrograms_path):
        for file_name in file_names:
            ...
            spectrogram = np.load(file_path)    #(n_bins, n_frames) -> (n_bins, n_frames,1)
            x_train.append(spectrogram)
            file_paths.append(file_path)
    x_train = np.array(x_train)
    x_train = x_train[...,np.newaxis]   # -> (3000, 256, 64, 1) : making it look like a grayscale image
